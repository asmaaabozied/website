...
class LaraVids {
    ...
    /**
     *
     * @param callback a two parameters callback: fun(file,message)
     */
    onFail(callback = null) {
        const _this = this;
        this.r.on('fileError', function (file, message) {
            // the file upload has resulted an error
            _this.failedFile = file;
            if (callback != null) {
    ...
    onSuccess(callback = null) {
        ...
        this.r.on('fileSuccess', function (file, message) {
            // the file upload has succeeded
            if (callback != null) {
                callback(file, message);
            }
            _this.r.removeFile(file);
        });
    }
    ...
    /**
     *
     * @param callback a two parameters callback: fun(file)
     */
    onAdd(callback = null) {
        this.r.on('fileAdded', function (file) {
             // this body is empty for further improvements
             if (callback != null) {
                 callback(file);
             }
         });
    ...
    /**
     *
     * @param callback a two parameters callback: fun(file, percentage)
     */
    onProgressUpdated(callback = null) {
        this.r.on('fileProgress', function (file) {
            // Handle progress for both the file and the overall upload
           const progressInPercentage = file.progress() * 100;
            if (callback != null) {
                callback(file, progressInPercentage);
            }
        });
    }
    ...
    retry() {
        this.failedFile.retry();
    }

    /**
     * upload with the additional data
     * @param data
     */
    upload(data) {
        const _this = this;
        $(data).each(function (i, field) {
            _this.r.opts.query[field.name] = field.value;
        });
        this.r.upload();
    }

    assignBrowse(element)
    {
