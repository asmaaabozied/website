            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('image') ? 'is-invalid' : '' }}" name="image_id" id="image_id">
                    @foreach($images as $id => $image)
                        <option value="{{ $id }}" {{ ($like->image ? $like->image->id : old('image_id')) == $id ? 'selected' : '' }}>{{ $image }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sound') ? 'is-invalid' : '' }}" name="sound_id" id="sound_id">
                    @foreach($sounds as $id => $sound)
                        <option value="{{ $id }}" {{ ($like->sound ? $like->sound->id : old('sound_id')) == $id ? 'selected' : '' }}>{{ $sound }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('video') ? 'is-invalid' : '' }}" name="video_id" id="video_id">
                    @foreach($videos as $id => $video)
                        <option value="{{ $id }}" {{ ($like->video ? $like->video->id : old('video_id')) == $id ? 'selected' : '' }}>{{ $video }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('comment') ? 'is-invalid' : '' }}" name="comment_id" id="comment_id">
                    @foreach($comments as $id => $comment)
                        <option value="{{ $id }}" {{ ($like->comment ? $like->comment->id : old('comment_id')) == $id ? 'selected' : '' }}>{{ $comment }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($like->user ? $like->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Like::LIKE_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('like_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="like_type_{{ $key }}" name="like_type" value="{{ $key }}" {{ old('like_type', $like->like_type) === (string) $key ? 'checked' : '' }}>
