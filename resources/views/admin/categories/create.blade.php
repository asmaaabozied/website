    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('icon_img') ? 'is-invalid' : '' }}" type="file" name="icon_img" id="icon_img" value="{{ old('icon_img', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category_type') ? 'is-invalid' : '' }}" name="category_type" id="category_type" required>
                    @foreach($category_types as $id => $category_type)
                        <option value="{{ $id }}" {{ old('category_type') == $id ? 'selected' : '' }}>{{ $category_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="parent_category" id="parent_category">
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ old('parent_category') == $id ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
